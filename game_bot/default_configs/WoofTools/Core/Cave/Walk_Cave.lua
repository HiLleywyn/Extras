...
CaveBot.doWalking = function()
  ...
  if dir then
    ...
    CaveBot.delay(player:getStepDuration(false, dir))
    return true
  end
  return false
end
...
onPlayerPositionChange(function(newPos, oldPos)
  ...
  if not isWalking or not expectedDirs[1] then
    ...
    walkPath = {}
    CaveBot.delay(g_game.getPing() + player:getStepDuration(false, dir))
    return
  end
...
CaveBot.walkTo = function(dest, maxDist, params)
  ...
  if CaveBot.Config.get("mapClick") then
    ...
    if ret then
      isWalking = true
      expectedDirs = path
      CaveBot.delay(math.max(g_game.getPing() + player:getStepDuration(false, dir), player:getStepDuration(false, dir) * 2))
    end
    return ret
  end
