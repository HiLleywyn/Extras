...
targetbotMacro = macro(100, function()
  ...
  if highestPriorityParams and not isInPz() then
    ...
    TargetBot.Creature.attack(highestPriorityParams, targets)
...
config = Config.setup("/Configs/Target", configWidget, "json", function(name, enabled, data)
  if not data then
    ui.status.right:setText("Off")
...
TargetBot.isActive = function() -- return true if attack takes place
